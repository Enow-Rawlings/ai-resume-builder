...
const ResumeBuilder = () => {
  ...
  const changeResumeVisibility = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || 'Failed to update resume visibility.')
    }
  }
  ...
  try {
    ...
    let response;
    const shouldUseFormData = typeof resumeData.personal_info.image === 'object' || removeBackground;
    ...
    if (shouldUseFormData) {
      ...
      response = await api.put('/api/resumes/update', {
        ...
        removeBackground: removeBackground ? 'yes' : 'no'
      }, {
        headers: { Authorization: token }
      });
    }
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-6'>
        <Link to={'/app'} className="inline-flex items-center text-slate-500 hover:text-slate-700 transition-all">
          <ArrowLeftIcon className='size-4' /> Back to Dashboard 
        </Link>
      </div>

      <div className='grid grid-cols-1 gap-8 lg:grid-cols-2 max-w-7xl mx-auto px-4 py-6 items-start'>
        {/* { Left Panel - Form} */}
        <div className='relative rounded-lg overflow-hidden'>
              <div className='bg-white rounded-lg shadow-sm border border-gray-200 p-6 pt-1'>
                ...
                {/* { Section Navigation} */}
                <div className='flex justify-between items-center mb-5 border-b border-gray-300 py-1'>
                  ...
                  <div className='flex items-center'>
                    ...
                    <button onClick={()=> setActiveSectionIndex((prevIndex)=> Math.min(prevIndex + 1, sections.length - 1))} className={`flex items-center gap-1 p-3 rounded-lg text-sm font-medium text-gray-600 hover:bg-gray-50 transition-all ${activeSectionIndex === sections.length -1 ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={activeSectionIndex === sections.length -1}> Next
                ...
                <button 
  onClick={() => {
    // ✅ Store the function execution promise in a variable
    const savePromise = saveResume();

    // ✅ Pass the promise reference directly into toast
    toast.promise(savePromise, {
      loading: 'Saving changes...',
      success: (data) => data?.message || 'Resume saved successfully!',
      error: (err) => err?.response?.data?.message || 'Failed to save resume. Please try again.'
    });
  }} 
            ...
            <div className='max-lg:mt-6'>
              <div className='relative w-full'>
                    <div className='absolute bottom-3 left-0 right-0 flex items-center justify-end gap-2'>
                      ...
                      <button onClick={downloadResume} className='flex items-center gap-2 py-2 text-xs bg-gradient-to-br from-green-100 to-green-200 text-green-600 rounded-lg ring-green-300 hover:ring transition-colors'>
