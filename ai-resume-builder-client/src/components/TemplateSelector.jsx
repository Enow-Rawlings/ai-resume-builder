...
//             id: "minimal-image",
//             name: "Minimal Image",
//             preview: "Minimal design with a singlr image and clean typography"
//         },
//         {
//             id: "minimal",
//             name: "Minimal",
...
//                 <Layout size={14} /> Template <span className='max-sm:hidden'></span>
//             </button>

//             {isOpen && (
//                 <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
//                     {templates.map((template)=>{
//                         <div key={template.id} onClick={()=>{onChange(template.id); setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? "border-blue-400 bg-blue-100" : "hover:bg-gray-100"}`}>
...
import { Layout, Check } from 'lucide-react' // Fixed: Added Check import
import React, { useState } from 'react'
...
const TemplateSelector = ({ selectedTemplate, onChange }) =>{
    ...
    return (
        <div className='relative'>
            ...
            {isOpen && (
                <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                    {/* Fixed: Added explicit return keyword inside map */}
                    {templates.map((template)=>{
                        return (
                            <div key={template.id} onClick={()=>{onChange(template.id); setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? "border-blue-400 bg-blue-100" : "hover:bg-gray-100"}`}>
